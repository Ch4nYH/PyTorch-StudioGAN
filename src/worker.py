...
from utils.ada import augment, initialize_ada, update_ada
...
class Worker(object):
    def __init__(self, cfgs, run_name, best_step, logger, writer, n_gpus, gen_model, dis_model, inception_model, Gen_copy,
                 Gen_ema, train_dataset, eval_dataset, train_dataloader, eval_dataloader, lt_sampler, grad_reg, G_optimizer,
                 D_optimizer, G_loss, D_loss, prev_ada_p, checkpoint_dir, mu, sigma, best_fid, best_fid_checkpoint_path,
                 default_device):
        self.cfgs = cfgs
    ...
    def prepare_worker(self):
        self.m_switch = change_model_mode(self.dis_model, self.gen_model, self.Gen_copy, self.prior, self.batch_size,
                                          self.z_dim, self.num_classes, self.default_device)

        check_flag_1(self.tempering_type, self.pos_collected_numerator, self.conditional_strategy, self.diff_aug, self.ada, self.mixed_precision,
        ...
        sampling_mode = sample_mode(self.dataset_name, self.conditional_strategy)
        self.fixed_zs, self.fixed_fake_labels = self.lt_sampler.sample(self.batch_size, self.truncated_factor, None, mode=sampling_mode)
    ...
    def train(self, current_step, total_step):
        ...
        if self.ada:
            self.ada_augment, self.ada_aug_p, self.ada_aug_step = initialize_ada(self.prev_ada_p, self.ada_target,
                                                                                 self.ada_length, self.default_device)
        else:
            self.ada_aug_p = 'No'
        ...
        while step_count <= total_step:
            ...
            for step_index in range(self.d_steps_per_iter):
                ...
                for acml_index in range(self.accumulation_steps):
                    ...
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        if self.zcr:
                            zs, fake_labels, zs_t = self.lt_sampler.sample(self.batch_size, 1, self.sigma_noise)
                        else:
                            zs, fake_labels = self.lt_sampler.sample(self.batch_size, 1, None)

                        if self.latent_op:
                            zs = self.grad_reg.latent_optimise(zs, fake_labels, self.latent_op_step, False)
                        ...
                        if self.gradient_penalty_for_dis:
                            dis_acml_loss += self.gradient_penalty_lambda*\
                                self.grad_reg.calc_derv4gp(real_images, fake_images, real_labels)
                        if self.deep_regret_analysis_for_dis:
                            dis_acml_loss += self.regret_penalty_lambda*\
                                self.grad_reg.calc_derv4dra(real_images, real_labels)
                        if self.ada:
                            self.ada_augment, self.ada_aug_p = update_ada(dis_out_real, self.ada_augment, self.ada_target,
                                                                          self.ada_aug_step, self.ada_aug_p, self.batch_size,
                                                                          self.default_device)
            ...
            for step_index in range(self.g_steps_per_iter):
                ...
                for acml_step in range(self.accumulation_steps):
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        if self.latent_op:
                            zs, transport_cost = self.grad_reg.latent_optimise(zs, fake_labels, self.latent_op_step, True)
            ...
            if step_count % self.print_every == 0 and self.logger:
                ...
                with torch.no_grad():
                    self.m_switch.eval(standing_statistics=False, standing_step=-1)
                    generator = self.Gen_copy if self.Gen_copy is not None else self.gen_model
                    generated_images = generator(self.fixed_zs, self.fixed_fake_labels)
            ...
            if step_count % self.save_every == 0 or step_count == total_step:
                if self.eval:
                    is_best = self.evaluate(step_count)
    ...
    def save(self, step, is_best):
        ...
        if self.Gen_copy is None:
            gen, dis = self.m_switch.save()
        else:
            gen, dis, gen_copy = self.m_switch.save()
    ...
    def evaluate(self, step):
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            self.m_switch.eval(standing_statistics=self.standing_statistics, standing_step=self.standing_step)
            generator = self.Gen_copy if self.Gen_copy is not None else self.gen_model

            fid_score, self.m1, self.s1 = calculate_fid_score(self.eval_dataloader, generator, self.inception_model, self.num_eval[self.eval_type],
                                                              self.lt_sampler, self.grad_reg, self.truncated_factor, self.latent_op, self.latent_op_step4eval,
                                                              self.default_device, self.mu, self.sigma, self.run_name)

            kl_score, kl_std = calculate_incep_score(self.eval_dataloader, generator, self.inception_model, self.num_eval[self.eval_type],
                                                     self.lt_sampler, self.grad_reg, self.truncated_factor, self.latent_op, self.latent_op_step4eval,
                                                     num_split, self.default_device)

            precision, recall, f_beta, f_beta_inv = calculate_f_beta_score(self.eval_dataloader, generator, self.inception_model, self.num_eval[self.eval_type],
                                                                           num_run4PR, num_cluster4PR, beta4PR, self.lt_sampler, self.grad_reg, self.truncated_factor,
                                                                           self.latent_op, self.latent_op_step4eval, self.default_device)
            ...
            if self.D_loss.__name__ != "loss_wgan_dis":
                real_train_acc, fake_acc = calculate_accuracy(self.train_dataloader, generator, self.dis_model, self.D_loss, self.num_eval[self.eval_type],
                                                              self.lt_sampler, self.grad_reg, self.truncated_factor, self.latent_op, self.latent_op_step4eval,
                                                              self.default_device, eval_generated_sample=True)
                ...
                if self.eval_type == 'train':
                    ...
                    real_eval_acc = calculate_accuracy(self.eval_dataloader, generator, self.dis_model, self.D_loss, self.num_eval[self.eval_type],
                                                       self.lt_sampler, self.grad_reg, self.truncated_factor, self.latent_op, self.latent_op_step4eval,
                                                       self.default_device, eval_generated_sample=False)
    ...
    def save_images(self, is_generate, png=True, npz=True):
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            if png:
                save_images_png(self.run_name, self.eval_dataloader, self.num_eval[self.eval_type], self.num_classes, generator, is_generate,
                                self.lt_sampler, self.grad_reg, self.truncated_factor, self.latent_op, self.latent_op_step4eval, self.default_device)
            if npz:
                save_images_npz(self.run_name, self.eval_dataloader, self.num_eval[self.eval_type], self.num_classes, generator, is_generate,
                                self.lt_sampler, self.grad_reg,  self.truncated_factor, self.latent_op, self.latent_op_step4eval, self.default_device)
    ...
    ################################################################################################################################
    def run_image_visualization(self, nrow, ncol):
        self.logger.info('Start visualizing images....')
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            self.m_switch.eval(standing_statistics=self.standing_statistics, standing_step=self.standing_step)
            generator = self.Gen_copy if self.Gen_copy is not None else self.gen_model

            sample_mode = "default" if self.conditional_strategy == "no" else "class_order_some"
            zs, fake_labels = self.lt_sampler.sample(self.batch_size, self.truncated_factor, None, sample_mode)
            if self.latent_op:
                zs = self.grad_reg.latent_optimise(zs, fake_labels, self.latent_op_step4eval, False)
    ...
    ################################################################################################################################
    def run_linear_interpolation(self, nrow, ncol, fix_z, fix_y):
    ...
    ################################################################################################################################
    def run_nearest_neighbor(self, nrow, ncol):
        ...
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            for c in tqdm(range(self.num_classes)):
                fake_images, fake_labels = generate_images_for_KNN(self.batch_size, c, generator, self.lt_sampler, self.grad_reg, self.truncated_factor,
                                                                   self.latent_op, self.latent_op_step4eval, self.default_device)
    ...
    ################################################################################################################################
    def run_frequency_analysis(self, num_images):
        self.logger.info('Start linear interpolation analysis....')
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            self.m_switch.eval(standing_statistics=self.standing_statistics, standing_step=self.standing_step)
            generator = self.Gen_copy if self.Gen_copy is not None else self.gen_model
            ...
            for i in range(num_batches):
                zs, fake_labels = self.lt_sampler.sample(self.batch_size, self.truncated_factor, None)
                if self.latent_op:
                    zs = self.grad_reg.latent_optimise(zs, fake_labels, self.latent_op_step4eval, False)
