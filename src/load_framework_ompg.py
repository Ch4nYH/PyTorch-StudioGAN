...
from train_eval_impg import Train_Eval
...
import copy
from utils.prune import pruning_generate, rewind_weight, see_remain_rate, pruning_generate_sn, pruning_generate_extract
...
def load_frameowrk(mask_path, mask_round, seed, disable_debugging_API, num_workers, config_path, checkpoint_folder, reduce_train_dataset, standing_statistics,
    ...
    initial_G_weight = copy.deepcopy(Gen.state_dict())
    initial_D_weight = copy.deepcopy(Dis.state_dict())
    if ema:
    ...
    if mask_path is not None:
        ...
        Gen, gen_masks = pruning_generate_extract(Gen, g_checkpoint['state_dict'], initial_G_weight)
        if ema:
            Gen_copy, _ = pruning_generate_extract(Gen_copy, g_checkpoint['state_dict'], initial_G_weight)
        Dis.load_state_dict(initial_D_weight)
